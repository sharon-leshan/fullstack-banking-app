...
const AccountForm = props => {
	...
	const handleUpdateAccount = ifsc => {
		...
		if (!allFieldsEntered) {
			setErrorMsg({ update_error: 'Please enter ifsc code.' });
		} else {
			setErrorMsg('');
	...
	const handleOnSubmit = e => {
		...
		if (!allFieldsEntered) {
			setErrorMsg({
				[selectedType === 'withdraw' ? 'withdraw_error' : 'add_error']:
			...
			if (selectedType === 'withdraw' && selectedAmount <= total_balance) {
				...
				setErrorMsg({
					[selectedType === 'withdraw' ? 'withdraw_error' : 'add_error']:
						"You don't have enoungh balance in your account",
				});
			}
		}
	};
			...
			<div className="edit-account col-md-6 offset-md-3">
				...
				<Form>
					{errorMsg && errorMsg.update_error && (
						<p className="errorMsg">{errorMsg.udpate_error}</p>
					)}
